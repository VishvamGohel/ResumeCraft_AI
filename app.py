# --- START OF FILE app.py (Simple Relative Paths) ---

import streamlit as st
import os
import json
# We are removing pathlib as it's causing issues in the cloud environment
...
def load_api_key():
    try:
        return st.secrets["COHERE_API_KEY"] # For Streamlit Cloud
    except (KeyError, FileNotFoundError):
        load_dotenv("app.env") # For local development
...
# --- NEW: Using simple, direct relative paths for assets ---
templates = {
...
# --- CSS Injection & Hero Section (No changes needed) ---
...
st.markdown("""<div class="hero-section">...</div>""", unsafe_allow_html=True) # Truncated for brevity
...
def create_template_card(template_name):
    ...
    # The st.image call now uses the simple relative path
...
if 'generation_request' in st.session_state and st.session_state.generation_request:
    ...
    with st.spinner("AI is crafting your signature resume..."):
        try:
            ...
            json_prompt = f"Generate a resume in structured JSON format...DETAILS TO PARSE: {user_data}"
            ...
            # We also simplify the template loader path
